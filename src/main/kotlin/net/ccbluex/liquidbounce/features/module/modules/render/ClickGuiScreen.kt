...
class ClickGuiScreen : Screen(Component.literal("ClickGUI")) {
    ...
    private fun getLayoutFile(): File {
        // 【修复】改用 LiquidBounce 官方配置目录, Android/Windows 都稳定
        return try {
    ...
    private fun saveLayout() {
        try {
            ...
            val panelsJson = panels.filter { it.category != null }.joinToString(",") { p ->
                ...
                // 【修复】scroll 安全格式化, 防止 NaN/Infinity 破坏 JSON
                val safeScroll = p.scrollOffset
                    .takeIf { it.isFinite() }
                    ?.coerceIn(0f, 100000f)
                    ?: 0f
                """{"tag":"$tag","x":${p.x.toInt()},"y":${p.y.toInt()},"collapsed":${p.collapsed},"scroll":${String.format("%.2f", safeScroll)}}"""
            }
    ...
    private fun loadLayout(): LayoutState {
        return try {
            ...
            for (match in panelRegex.findAll(content)) {
                ...
                val scroll = match.groupValues[6].takeIf { it.isNotEmpty() }?.toFloatOrNull() ?: 0f
