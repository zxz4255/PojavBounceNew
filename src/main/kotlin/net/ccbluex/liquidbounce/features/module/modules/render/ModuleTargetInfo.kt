            ...
            if (showHead) {
                ...
                if (texture != null) {
                    ...
                    if (damageTint && entity.hurtTime > 0 && entity.hurtDuration > 0) {
                        val factor = entity.hurtTime / entity.hurtDuration.toFloat()
                        val a = (0x70 * factor * alpha).roundToInt().coerceIn(0, 255)
                        if (a > 0) {
                            context.fill(headX, headY, headX + size, headY + size, (a shl 24) or 0x00FF0000)
                        }
                    }
                }
            }
