/*
 * Mode under NoSlow → Consume. Do NOT register in ModuleManager.
 */
...
/**
 * Opal GrimFastNoSlow port — offhand swap consume no-slow.
 */
class NoSlowConsumeGrimFast(override val parent: ModeValueGroup<*>) : Mode("GrimFast") {
    ...
    private val gameTickHandler = handler<GameTickEvent> {
        ...
        val hasScreen = runCatching { mc.gui.screen() != null }.getOrDefault(false)
        if (hasScreen) {
            resetOffhandState()
            return@handler
        }

        if (useState == UseState.WAITING && ++waitTicks >= waitFallbackTicks) {
            startSwap()
        }
        if (useState == UseState.SWAPPING && ++swapTicks >= swapFallbackTicks) {
            beginUsing()
        }
        if (useState == UseState.USING) {
    ...
    private fun resetOffhandState() {
        ...
        while (pongQueue.isNotEmpty()) {
            runCatching { sendPacketSilently(pongQueue.removeFirst()) }
        }
