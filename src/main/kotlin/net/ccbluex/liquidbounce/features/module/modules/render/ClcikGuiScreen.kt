...
class ClickGuiScreen : Screen(Component.literal("ClickGUI")) {
    ...
    // ==================== Colors (亮紫主题, 匹配参考图) ====================
    private val ACCENT = 0xFFC77DFFL.toInt()
    private val ACCENT_DARK = 0x66C77DFFL.toInt()
    private val BG = 0xE01A1A22L.toInt()
    private val PANEL_BG = 0xD81C1C24L.toInt()
    private val TEXT get() = ModuleClickGui.getTextColor()
    private val TEXT_BRIGHT = 0xFFC77DFFL.toInt()
    ...
    private val CATEGORY_BG = 0x806B2FA8L.toInt()         // 标题栏背景 alpha=80 紫色
    private val CATEGORY_LINE = 0xFFC77DFFL.toInt()       // 标题栏底部亮紫细条
    ...
    private val SCROLL_TRACK = 0x18C77DFFL.toInt()
    private val SCROLL_THUMB = 0x50C77DFFL.toInt()
    private val SCROLL_THUMB_HOVER = 0x78C77DFFL.toInt()
    private val EXPANDED_BG = 0x0AC77DFFL.toInt()
    private val GROUP_BG = 0x0CC77DFFL.toInt()
    private val GROUP_LINE = 0x14C77DFFL.toInt()
    ...
    override fun extractRenderState(ctx: GuiGraphicsExtractor, mouseX: Int, mouseY: Int, delta: Float) {
        ...
        if (isSearching) {
            ...
            val h = (sh * 0.85f).coerceAtMost(PANEL_MAX_H.toFloat())
            val x = (sc - w) / 2f
            val y = (sh - h) / 2f
            ...
            val panelH = (sh * 0.85f).coerceAtMost(PANEL_MAX_H.toFloat())
        ...
        for (panel in panelRenderList) {
            ...
            if (isSearching) {
                ...
                // 【新增】搜索标题栏背景面板 alpha=80 紫色 + 底部亮紫细条
                fillRect(ctx, px, py, px + pw, py + HEADER_H, CATEGORY_BG)
                ...
                // 【新增】标题栏背景面板 alpha=80 紫色 (通宽, 覆盖整个标题栏)
                fillRect(ctx, px, py, px + pw, py + HEADER_H, CATEGORY_BG)
                drawText(ctx, font, "§l$arrow${category.tag}", (px + 8f).toInt(), (py + 5f).toInt(), CATEGORY_TITLE)
                // 【修改】标题栏底部亮紫细条 (通宽 1px 亮紫, 位于标题栏底边)
                fillRect(ctx, px, py + HEADER_H - 1f, px + pw, py + HEADER_H, CATEGORY_LINE)
            }
