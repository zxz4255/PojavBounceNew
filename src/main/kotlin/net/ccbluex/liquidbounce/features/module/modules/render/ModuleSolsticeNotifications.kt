...
object ModuleSolsticeNotifications : ClientModule(
    ...
    // —— 动画 ——
    // 原版: deltaTime * 5.0f (线性趋近, 有拖尾)。本版改为匀速补全(无拖尾, 到点即止)。
    private val animationSpeed by float("Animation Speed", 12f, 1f..1000f)
    ...
    private fun renderNotifications(ctx: GuiGraphicsExtractor, dt: Float) {
        ...
        for (n in notifications) {
            ...
            // 原版: 时间推进 + currentDuration 动画
            n.timeShown += dt
            n.isTimeUp = n.timeShown >= n.duration
            // 【修复】匀速补全 (无拖尾): 出现 0→1 每帧 +dt*speed, 收回 1→0 每帧 -dt*speed
            val animStep = dt * animationSpeed
            if (n.isTimeUp) {
                n.currentDuration = (n.currentDuration - animStep).coerceAtLeast(0f)
            } else {
                n.currentDuration = (n.currentDuration + animStep).coerceAtMost(1f)
            }
